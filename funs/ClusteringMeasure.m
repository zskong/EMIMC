function [result,res] = ClusteringMeasure(Y, predY)
...
[F,P,R] = compute_f(Y,predY);

        [nmi avgent] = compute_nmi(Y,predY);
        if (min(Y)==0)
            [AR,RI,MI,HI]=RandIndex(Y+1,predY);
        else
            [AR,RI,MI,HI]=RandIndex(Y,predY);
 end  


result = [ACC MIhat Purity F P R AR];
